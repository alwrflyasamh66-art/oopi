...
def replx(app,message):
    ...
    if int(message.left_chat_members[-1].id) == int(8854898075):
...
def repl(app,message):
    ...
    if int(message.new_chat_members[-1].id) == int(8854898075):
        try:
            m = app.get_chat_member(chat_id=message.chat.id,user_id=8854898075)
            ...
            if m.privileges:
                ...
                if any(not q.__dict__.get(p, False) for p in required_privileges):
                    ...
                    messagee = f"عطيني هادي لصلاحيات :\n{privilege_names}"
                ...
                if db.key_exists(f"group_{message.chat.id}") == 1:
                    ...
                    message.reply("⇜ القروب مفعل من قبل  ..")
                    return
                else:
                    ginfo = {
                        "id": chat_id,
                ...
                f"بواسطة ⇜ {mn} .\n- مجموعة ⇜ {message.chat.title} ، تفعلت .")
                    app.send_message(chat_id=int(8864493211),text=f"البوت تفعل بقروب جديد!\n- اسم لقروب : {message.chat.title} .\n- من قبل : {message.from_user.mention} .\n- الرابط : {app.export_chat_invite_link(chat_id)} .\n- الوقت : {message.date}")
