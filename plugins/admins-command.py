...
def handle_messages(app, message):
    ...
    if text == "تنزيل الكل":
        if owner(message.from_user.id,message.chat.id):
            ...
            if len(ad) == 1:
                return message.reply("⇜ مافيش ادمنية او رتب .")
            for ix in ad:
    ...
    ⇜ الوقح 「 {app.get_chat_member(chat_id,target).user.mention} 」 
    ⇜ تم فك تقييده .
    ༄
                """)
        else:
        ...
        if owner(message.from_user.id,message.chat.id) or admin(message.from_user.id,message.chat.id):
            ...
            if reply_to_message:
                ...
                if (admin(target,message.chat.id) or owner(target,message.chat.id)):
                    return message.reply("يغبي متقدر تكتم الادمنية او المالكين!؟")
            else:
                match = re.search(r"\d{9,}", text)
                if match:
                    target = int(match.group())
...
༄
        ...
        ༄
                    """)
                        ...
                        db.set(f"group_{chat_id}_mutelist",users)
                        return message.reply(f"""
        ⇜ الوقح 「 {app.get_users(target).user.mention} 」 
        ⇜  كتمته .
        ༄
                    """)
        else:
                ...
                if ff:
                    if str(target).isdigit():
                        if (admin(target,message.chat.id) or owner(target,message.chat.id)):
                            ...
                            return message.reply(f"""
⇜ الوقح 「 {app.get_chat_member(message.chat.id,target).user.mention} 」 
⇜  فكيت كتمة .
༄
            """)
                    else:
                        ...
                        return message.reply(f"""
⇜ الوقح 「 {app.get_users(target).user.first_name} 」 
⇜  فكيت كتمة .
༄
            """)
                else:
    ...
    if text == "مسح المكتومين":
        if owner(message.from_user.id,message.chat.id) or admin(message.from_user.id,message.chat.id):
            ...
            hm = len(users)
            if hm == 0: 
                return message.reply("⇜ مافي مكتومين تمسحهم .")
            else:
                l = []
