...
༄ 
        """.replace("ChatMemberStatus.MEMBER",
                    'عضو').replace("ChatMemberStatus.ADMINISTRATOR", 'ادمن').replace(
                        ...
                        "المالك").replace("None", "مافيش يوزر").replace("ChatMemberStatus.RESTRICTED",'عضو')
                message.reply(infos)
            else:
                chat_member = app.get_chat_member(message.chat.id,
...
༄ 
    ...
    if text == 'صورته' and message.reply_to_message and message.reply_to_message.from_user:
        m = message
        if not m.reply_to_message.from_user.photo:
            return m.reply(f' مانقدر نجيب صورته يمكن حاظرني')
        else:
    ...
    if text == 'صورتي':
        ...
        if not m.from_user.photo:
            return m.reply(f' ماقدر اجيب صورتك ارسل نقطه خاص وارجع جرب')
        else:
        ...
        if message.from_user.id == admin:
            if db.get(f"running_rolet_{message.chat.id}"):
                ...
                if len(players) == 2 or len(players) >2:
                    ...
                    return message.reply("مافيش ناس ..")
            else:
                return message.reply("مافيه روليت")
        return
    command = text
...
""".replace("ChatMemberStatus.MEMBER",
            'عضو').replace("ChatMemberStatus.ADMINISTRATOR", 'ادمن').replace(
            "ChatMemberStatus.OWNER",
            "المالك").replace("None", "مافيش يوزر").replace("ChatMemeberStatus.RESTRICTED","عضو").replace(
...
""".replace("ChatMemberStatus.MEMBER",
                'عضو').replace("ChatMemberStatus.ADMINISTRATOR", 'ادمن').replace(
                "ChatMemberStatus.OWNER",
                "المالك").replace("None", "مافيش يوزر")
                message.reply(infos)
            except Exception as e:
                #print(e)
                message.reply("مالقيت احد")
    if text == "بايو":
        if message.reply_to_message:
