...
BOT_TOKEN  = "8876817025:AAHS5ymWu8z6b3WYtQ8mfcraUdnYAqmyui"  # توكن بوت المصنع
...
def _screen_running(username):
    ...
    for raw_line in result.stdout.splitlines():
        ...
        # صيغة screen المعتادة: <pid>.<session_name> <status> <date>
...
async def callback_handler(client, query):
    ...
    # ── قائمة البوتات ──
        ...
        text = (f"<b>إدارة البوت @{record[0]}</b>\n\n"
                ...
                f"اسم screen: <code>{_screen_name(record[0])}</code>\n"
...
async def _step_ask_token(client, message):
    ...
    if message.from_user.id in DEVS:
        ...
        # المستخدم العادي ينصّب البوت لنفسه
...
async def _do_install(client, message, info: dict):
    ...
    # ── تعديل المتغيرات باستخدام regex (يتعامل مع المسافات والتعليقات) ──
    with open(dest_file, "r", encoding="utf-8") as f:
        content = f.read()

    # استبدال BOT_TOKEN مع أي عدد مسافات وتعليقات
    content = re.sub(
        r'BOT_TOKEN\s*=\s*"[^"]*"',
        f'BOT_TOKEN = "{token}"',
        content
    )
    # استبدال ADMIN_ID مع أي عدد مسافات وتعليقات
    content = re.sub(
        r'ADMIN_ID\s*=\s*\d+',
        f'ADMIN_ID = {admin_id}',
        content
    )
    # استبدال INSTALL_DEADLINE مع أي عدد مسافات وتعليقات
    content = re.sub(
        r'INSTALL_DEADLINE\s*=\s*\d+',
        f'INSTALL_DEADLINE = {deadline_ts}',
        content
    )
    ...
    with open(dest_file, "w", encoding="utf-8") as f:
        ...
        # ── تسجيل البوت ثم تشغيله في screen ──
    installer = message.from_user.username or str(message.from_user.id)
    _run_screen_record([username, admin_id, token, installer, deadline_ts])
    Bots.append([username, admin_id, token, installer, deadline_ts])
    _save_bots()
...
def _run_screen(session_name: str, script_path: str):
    """تشغيل البوت في screen:
    screen -dmS {يوزر البوت} bash -c 'cd {مجلد البوت} && python3 source.py'
    """
    bot_dir = os.path.dirname(script_path)

    # إيقاف الجلسة القديمة إن وجدت
    os.system(f"screen -XS {session_name} quit 2>/dev/null")
    time.sleep(1)

    # إنشاء screen مع اسم البوت، الانتقال لمجلده، وتشغيل source.py
    _run_screen_record([session_name, 0, "", "", 0])
    time.sleep(0.5)

    # التحقق أن الجلسة شغالة
    check = os.system(f"screen -ls | grep -q {session_name} 2>/dev/null")
    if check != 0:
        print(f"⚠️ فشل إنشاء screen للبوت {session_name} — تأكد من تثبيت screen")
    else:
        print(f"✅ البوت {session_name} يعمل في screen")
...
async def _do_renew(client, message, username: str, extra_days: int):
    """تنفيذ التجديد: تعديل الملف + إعادة تشغيل screen + إشعار الأدمن."""
    ...
    # تعديل INSTALL_DEADLINE في الملف (باستخدام regex للتعامل مع المسافات)
    with open(dest_file, "r", encoding="utf-8") as f:
        content = f.read()
    ...
    if new_content == content:
        return await message.reply_text(
            "⚠️ لم يتم العثور على متغير `INSTALL_DEADLINE` في الملف."
        )
    ...
    # إعادة تشغيل screen بالبوت المجدَّد
    _run_screen_record(bot_entry)

    # إشعار أدمن البوت عبر البوت المنصَّب نفسه
    try:
        notify_bot = Client(
