    ...
    for root in ("database", "vcf_files", "."):
        ...
        for path in Path(root).glob("*.tmp"):
            try:
                ...
                pass
    backup_path = Path("روسيBackUp.json")
    try:
...
class Custem :
    async def add_users_contact(self, group_link: str, bot: Client, nmcount: int, resume_state=None):
        ...
        await abot.send_message(
            ...
            (
                f"{status}\n"
                "ᯓ  𓏺˛ ᔆᴼᵁᴿᶜᴱ ᴬᵂᴬᴮ.<tg-emoji emoji-id='4958479549265347295'>⚡️</tg-emoji> - إشعـــــار النقل من الجهات ⛞\n"
                "⋆─┄──┄─┄─┄─┄┄─┄┄─┄─⋆\n\n"
    ...
    async def ADDuser(self, inGRob, grop2, bot: Client, nmcount, resume_state=None):
        ...
        await abot.send_message(
            Config.OWNER_ID,
            (
                f"سـورس𝑹𝑶𝑺𝑺𝑬.<tg-emoji emoji-id='4958479549265347295'>⚡️</tg-emoji> - إحصــائيات النقل الظـــاهر {status} ⛞\n"
                "⋆─┄──┄─┄─┄─┄┄─┄┄─┄─⋆\n\n"
                f"عدد الاضافات المؤكدة فعلياً: {operation_metrics(progress_state, total_to_add)[0]} عضو\n"
                f"عدد الاضافات الفاشله: {operation_metrics(progress_state, total_to_add)[1]} عضو\n"
                f"موجودون مسبقاً (تم تجاهلهم): {len(not_add_users)} عضو\n"
    ...
    async def add_users_hide(self, group_link: str, bot: Client, nmcount: int):
        ...
        await abot.send_message(
            Config.OWNER_ID,
            "ᯓ  𓏺سـورس 𝑹𝑶𝑺𝑺𝑬 .<tg-emoji emoji-id='4958479549265347295'>⚡️</tg-emoji> - إشعـــــار النقل من الملفات ⛞\n"
            "⋆─┄──┄─┄─┄─┄┄─┄┄─┄─⋆\n\n"
            f"عدد الاضافات المؤكدة فعلياً: {operation_metrics(progress_state, total_to_add)[0]} عضو\n"
            f"عدد الاضافات الفاشله: {operation_metrics(progress_state, total_to_add)[1]} عضو\n"
            f"موجودون مسبقاً (تم تجاهلهم): {len(not_add_users)} عضو\n"
    ...
    if True:
        ...
        await abot.send_message(
            ...
            text=(
                ...
                f"<a href='https://t.me/a990099aa'>سورس 𝑹𝑶𝑺𝑺e</a>\n\n\n"
...
async def call_handler(client, call):
    ...
    if data == "back":
            ...
            return await abot.edit_message_text(
                ...
                text=(
                    ...
                    f"عزيزي {fname} في بوت نقل الأعضاء المتطور V5.7 Pro !\n\n"
                    f"قناة السورس<tg-emoji emoji-id='5233546443360318055'>⚡</tg-emoji><tg-emoji emoji-id='5397857289216484878'>⚡</tg-emoji> "
                    f"<a href='https://t.me/source_abdo'>A3Do SoUrCe</a>\n\n\n"
                    f"اختر من الأزرار في الاسفل "
                    f"<tg-emoji emoji-id='5328306457135821693'>👇</tg-emoji>"
                ),
                reply_markup=inline_keyboard,
    ...
    # ===== فحص صحة الحسابات (الجلسة + قيود @SpamBot) =====
        ...
        report_text = (
            f"ᯓ  𓏺˛ 𝑹𝑶𝑺𝑺𝑬.<tg-emoji emoji-id='4958479549265347295'>⚡️</tg-emoji> - نتيجة فحص الحسابات ⛞\n"
            "⋆─┄──┄─┄─┄─┄┄─┄┄─┄─⋆\n\n"
    ...
    # باك اب حسابات
        ...
        await abot.edit_message_text(
            ...
            message_id=call.message.id,
            text=(
                "<tg-emoji emoji-id='5472164874886846699'>✨</tg-emoji> تم حفظ البيانات في ملف ROSSEBackUp.json بنجاح. "
    ...
    # رفع النسخة الاحتياطية
        ...
        await bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.id,
            text="قم برفع ملف النسخة الاحتياطية (ROSSEBackUp.json)"
        )
    ...
    # نقل الأعضاء ظاهر
        ...
        await bot.edit_message_text(
            ...
            text=(
                ...
                "<a href='https://t.me/a990099aa'>ᯓ  𓏺˛ 𝑹𝑶𝑺𝑺𝑬.<emoji id='4958479549265347295'>⚡️</emoji></a>"
            ),
        parse_mode=enums.ParseMode.HTML)
...
async def handle_check_link(client, message, orig_call):
    ...
    report = (
        "𝑹𝑶𝑺𝑺𝑬.<tg-emoji emoji-id='4958479549265347295'>⚡️</tg-emoji> - فاحص الحسابات  ⛞\n"
        "⋆─┄──┄─┄─┄─┄┄─┄┄─┄─⋆\n\n"
...
async def start_add_con(client, message, group_link):
    ...
    notif_msg = await abot.send_message(chat_id=message.chat.id, text=
        "𝑹𝑶𝑺𝑺𝑬.<tg-emoji emoji-id='4958479549265347295'>⚡️</tg-emoji> - إشعـــــار النقل جهات الاتصال⛞\n"
        "⋆─┄─┄─┄─┄─┄┄─┄┄─┄─⋆\n\n"
        f"الطلب : {count} عضو\n"
...
async def start_adding(client, message, group_link, selected_files=None):
    ...
    return
    notif_msg = await abot.send_message(chat_id=message.chat.id, text=
        "𝑹𝑶𝑺𝑺𝑬.<tg-emoji emoji-id='4958479549265347295'>⚡️</tg-emoji> - إشعـــــار النقل الاعضاء محفوظه⛞\n"
        "⋆─┄─┄─┄─┄─┄┄─┄┄─┄─⋆\n\n"
...
async def statement2(client, message, user_info: dict):
    ...
    return
    notif_msg = await abot.send_message(message.chat.id,f"""𝑹𝑶𝑺𝑺𝑬.<tg-emoji emoji-id='4958479549265347295'>⚡️</tg-emoji> - إشعـــــار النقل الظاهر ⛞
⋆─┄─┄─┄─┄─┄┄─┄┄─┄─⋆
...
async def statement2hide(client, message, user_info: dict):
    ...
    notif_msg = await abot.send_message(
        message.chat.id,
        (
            "ᯓ  𓏺˛ ᔆᴼᵁᴿᶜᴱ ᴬᵂᴬᴮ.<tg-emoji emoji-id='4958479549265347295'>⚡️</tg-emoji> - إشعـــــار النقل المخفي ⛞\n"
            "⋆─┄─┄─┄─┄─┄┄─┄┄─┄─⋆\n\n"
            f"المتاحين للاضافة : {numUser} عضو\n"
...
async def AddBackupAccounts(client, message):
    ...
    if message.document and (message.document.file_name or "").lower().endswith(".json"):
        await message.download("./ROSSEUp.json")
        try:
            with open("./ROSSEUp.json", "r", encoding="utf-8") as f:
                data = json.load(f)
